# EComCalendar — тестовое задание AI Developer

Сервис бронирования встреч между сотрудниками с проверкой пересечений слотов.
REST API + SQLite + проверка конфликтов + тесты + telegram бот.
...
3. **Временная зона** — всё хранится в UTC.
4. **Миграция на Postgres** — заменить `DATABASE_URL` в `.env` на `postgresql+asyncpg://...`.
5. **Понедельник — начало недели** — функция `get_meetings_for_week` принимает дату понедельника и возвращает встречи за 7 дней.
...
8. **Старые встречи не очищаются, и пользователь может создавать встречи на даты которые прошли** - Недопустимо в продакшене, но для тестового задания не проблема.
